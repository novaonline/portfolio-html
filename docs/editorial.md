...
Choose one overview explaining the system or focused diagrams placed beside the paragraphs that explain each part. Do not collect a breakdown into an isolated block. Carry component symbols into the prose using shared inline shapes; the homepage's `DiagramIdentity` component provides the same identity in Astro prose and diagram nodes.

Before accepting a visual, state what an engineer or fellow architect learns from it. Showing a circle travel between boxes is insufficient on its own. Reveal a relationship, responsibility, decision, consequence or tradeoff. A static diagram may be enough; add interactions only when changing something exposes meaningful behavior. Evaluate explanatory value before palette or motion fidelity. The greenfield overview illustrates this with an Analytics pause between batches: the current batch finishes, then saved items and other consumers progress while observations lag. Resume processing drains the queued events in batches. Worker and Analytics collect up to three events or dispatch a partial batch after two illustrative seconds; this is scenario-specific, not a general visual-profile rule.
